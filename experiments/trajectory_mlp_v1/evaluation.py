...
class Evaluator:
    ...
    def _add(stats, target, prediction, mask, support, fallback, group_has_visible):
        """Vectorized float64 CPU reductions; one group remains one statistic."""
        def array(x):
            return x.detach().cpu().numpy() if torch.is_tensor(x) else x
        target, prediction = (np.asarray(array(x), dtype=np.float64) for x in (target, prediction))
        mask, support, group_has_visible = (array(x) for x in (mask, support, group_has_visible))
        counts = mask.sum(-1)
        traj = counts > 0
        ntraj = traj.sum(-1)
        active = ntraj > 0
        # Select before subtracting so irrelevant NaNs/Infs cannot contaminate sums.
        error = np.where(mask, prediction, 0.) - np.where(mask, target, 0.)
        absolute = np.abs(error)
        totals = error.sum(-1)
        per_traj = absolute.sum(-1) / np.maximum(counts, 1)
        stats['unscorable'] += int((~group_has_visible).sum())
        stats['bins'] += int(counts.sum())
        stats['abs_sum'] += float(absolute.sum())
        stats['sq_sum'] += float(np.square(error).sum())
        supported = int((mask & support[:, None, :]).sum())
        stats['same_supported'] += supported
        stats['fallback'] += int(counts.sum()) - supported
        stats['trajectories'] += int(traj.sum())
        stats['total_abs_sum'] += float(np.abs(totals).sum())
        stats['total_sq_sum'] += float(np.square(totals).sum())
        stats['groups'] += int(active.sum())
        stats['group_mae_sum'] += float((per_traj.sum(-1) / np.maximum(ntraj, 1))[active].sum())
    ...
    def update(self, prediction_seconds_or_none, batch):
        # Transfer each tensor once; all metric/audit loops are CPU-only.
        batch = {k: v.detach().cpu() if torch.is_tensor(v) else v for k,v in batch.items()}
        if prediction_seconds_or_none is not None:
            prediction_seconds_or_none = prediction_seconds_or_none.detach().cpu()
    ...
    def _add_strata(self, target, baseline, prediction, mask, support, group_has_visible, batch):
        """Batch reductions; length strata retain the legacy per-trajectory unit."""
        target, baseline = target.numpy(), baseline.numpy()
        prediction = prediction.numpy() if prediction is not None else None
        mask, support, has = mask.numpy(), support.numpy(), group_has_visible.numpy()
        valid = batch['bin_valid'].numpy()
        ratio = batch['x'][..., 1].numpy()
        counts = valid.sum(-1)
        active = mask.any(axis=(1, 2))
        B, M, N = mask.shape

        def add(category, value, selected, indices=None):
            if not selected.any():
                return
            idx = np.flatnonzero(selected.any(axis=(1, 2))) if indices is None else indices
            if indices is None:
                selected = selected[idx]
            entry = self._strata[category][str(value)]
            self._add(entry['baseline'], target[idx], baseline[idx], selected, support[idx], None, has[idx])
            if prediction is not None:
                self._add(entry['ours'], target[idx], prediction[idx], selected, support[idx], None, has[idx])

        kval = batch.get('K', torch.full((B,), M)).numpy()
        for k in np.unique(kval[active]):
            idx = np.flatnonzero(active & (kval == k))
            add('K', int(k), mask[idx], idx)
        # Legacy length strata counted each hidden trajectory as a statistical
        # group, even if several trajectories shared a length in the same group.
        hidden = mask.any(-1)
        for length in np.unique(counts[hidden]):
            bi, ri = np.nonzero(hidden & (counts == length))
            selected = mask[bi, ri][:, None, :]
            entry = self._strata['valid_bin_length'][str(int(length))]
            self._add(entry['baseline'], target[bi, ri][:, None, :], baseline[bi, ri][:, None, :],
                      selected, support[bi], None, has[bi])
            if prediction is not None:
                self._add(entry['ours'], target[bi, ri][:, None, :], prediction[bi, ri][:, None, :],
                          selected, support[bi], None, has[bi])
        fullratio = (counts == N) & (np.max(np.abs(ratio-1)*valid, axis=-1) < 1e-6)
        add('same_bin_support', 'same_bin', mask & support[:, None, :])
        add('same_bin_support', 'fallback', mask & ~support[:, None, :])
        add('target_ratio', 'full', mask & (np.abs(ratio-1) < 1e-6))
        add('target_ratio', 'partial', mask & (np.abs(ratio-1) >= 1e-6))
        add('full50', 'full50', mask & (counts == N)[..., None])
        add('full50', 'partial', mask & (counts != N)[..., None])
        add('fullratio', 'full', mask & fullratio[..., None])
        add('fullratio', 'partial', mask & ~fullratio[..., None])
