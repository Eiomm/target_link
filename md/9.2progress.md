...
## 11. 数据稀疏性诊断与扩展验证（2026-09-04）

### 11.1 K 稀疏根因：part 文件按轨迹分片

单文件 `part-00002.parquet` 只含全小时 241,490 条轨迹中的 **2,781 条（1.2%）**，展开成 22.2 万样本。
因此单文件下 (link,window) 的 K 被压低 ~50-85 倍，**K=1 占 58.6% 是抽样伪影，不是真实稀疏度**。

### 11.2 4 文件验证结果（08 时 part 02/04/11/14）

| 指标 | 单文件 (1 part) | 4 文件 (4 parts) |
|---|---|---|
| 样本数 | 222,063 | 553,925 |
| K=1 占比 | 60.0% | 48.9% |
| K 中位数 | 1.0 | 2.0 |
...
**假设成立**：1→4 文件 K=1 从 60%→49%。线性外推全 15 文件预计 K=1 降到 15-20%。
200m 切分不影响 K（样本 100% 覆盖其 link 全部 sub，sub 级 K ≡ link 级 K）。

### 11.3 内存约束（重要）

`ingest.py` 对全量 15 文件（11 亿行）用 `pd.concat` 累积 bins 会 **OOM**——11 亿行峰值远超可用内存，
触发 cgroup OOM killer，**被杀的是 orangefs（NFS 守护进程，37GB 虚拟内存），影响全机文件访问**。

已写 `tools/ingest_streaming.py`：逐文件处理、bins 分片落盘、只累积 samples 小表，峰值内存=单文件（~2GB）。
`build_profiles.py` 尚需配套流式改造（当前全量 groupby/cumsum 同样会爆）。全量扩展时启用。

### 11.4 当前数据产物

| 目录 | 规模 | 用途 |
|---|---|---|
| `data/processed/` | 22.2万样本（1文件） | 历史单文件产物，保留对比 |
| `data/processed_4f/` | 55.4万样本 / 65.9万 profiles（4文件） | **当前开发用** |
| `data/raw_hdfs/event_hour=2026081708/` | 8.4GB / 15文件 / 11亿行 | 08时全量，后续流式扩展用 |

### 11.5 build_profiles.py 修复

多文件合并后有 1 个重复 sample_id（分片边界泄漏），`set_index` 要求唯一索引会报 `InvalidIndexError`。
已改用 `drop_duplicates("sample_id")` 去重（重复行内容完全一致）。
