"""Plot the per-trajectory 10 m passage-time distribution of one traffic cell.
...
Example (server):
    ...
    spark-submit tools/plot_link_bin_times.py \
      --corpus hdfs://DClusterNmg3/user/bigdata-dp/user/junao/target_link/corpus_v1 \
      --link 123456789 --out runtime/link_123456789.svg

Example (local raw reference package, no Spark required):

    runtime/window-dev/bin/python tools/plot_link_bin_times.py \
      --raw-parquet data/ref_link_125121851/samples_flat.parquet \
      --out runtime/link_125121851.svg
...
def render_svg(matrix, sample_ids, entry_times, meta, per_bin, out_path, max_rows=300):
    ...
    cw, rh = plot_w / N_BINS, heat_h / len(shown)
    ...
    add('<text class="sub" x="%d" y="58">%s · colour P%.0f–P%.0f = %.3g–%.3g s · grey = invalid/missing</text>' %
        (left, _esc(metric_label), meta["lower_percentile"], meta["upper_percentile"], lo, hi))

    # Compact continuous legend.
    lx, ly, lw = width - right - 250, 78, 250
    for i in range(100):
        v = lo + (hi - lo) * i / 99
        add('<rect x="%.2f" y="%d" width="%.2f" height="10" fill="%s"/>' %
            (lx + lw * i / 100, ly, lw / 100 + 0.2, _colour(v, lo, hi)))
    add('<text class="axis" x="%.1f" y="104" text-anchor="start">%.3g s</text>' % (lx, lo))
    add('<text class="axis" x="%.1f" y="104" text-anchor="end">%.3g s</text>' % (lx + lw, hi))

    # Heatmap.
    for i, row in enumerate(shown):
        for j, value in enumerate(row):
            y = heat_top + i * rh
            add('<rect x="%.3f" y="%.3f" width="%.3f" height="%.3f" fill="%s">'
                '<title>%s · bin %d (%d–%dm) · %s</title></rect>' %
                (left + j * cw, y, cw + 0.15, rh + 0.15, _colour(float(value), lo, hi),
                 _esc(shown_ids[i]) + " · " +
                 datetime.fromtimestamp(float(shown_entry[i]), BEIJING).strftime("%H:%M:%S"),
                 j, j * 10, (j + 1) * 10,
                 "missing" if not math.isfinite(float(value)) else "%.4g s" % value))
    ...
    for i in tick_rows:
        ...
        add('<text class="axis" x="%d" y="%.2f" text-anchor="end">%s</text>' %
            (left - 9, y + 4, label))
    for j in range(0, N_BINS + 1, 5):
        x = left + j * cw
        add('<line class="grid" x1="%.2f" y1="%d" x2="%.2f" y2="%.1f"/>' %
            (x, heat_top, x, heat_top + heat_h))
        add('<text class="axis" x="%.2f" y="%.1f" text-anchor="middle">%d</text>' %
            (x, heat_top + heat_h + 19, j * 10))
    ...
    for tick in np.linspace(dist_lo, dist_hi, 5):
        ...
        add('<text class="axis" x="%d" y="%.2f" text-anchor="end">%.2g</text>' %
            (left - 9, y + 4, tick))
    upper = [(left + (d["bin_pos"] + 0.5) * cw, y_of(d["p75_s"]))
             for d in per_bin if math.isfinite(d["p75_s"])]
    lower = [(left + (d["bin_pos"] + 0.5) * cw, y_of(d["p25_s"]))
             for d in reversed(per_bin) if math.isfinite(d["p25_s"])]
    if upper and lower:
        pts = " ".join("%.2f,%.2f" % p for p in upper + lower)
        add('<polygon points="%s" fill="#8ab4d6" fill-opacity="0.35"/>' % pts)
    med = [(left + (d["bin_pos"] + 0.5) * cw, y_of(d["p50_s"]))
           for d in per_bin if math.isfinite(d["p50_s"])]
    if med:
        add('<polyline points="%s" fill="none" stroke="#315f86" stroke-width="2.2"/>' %
            " ".join("%.2f,%.2f" % p for p in med))
    for d in per_bin:
        if math.isfinite(d["p50_s"]):
            x, y = left + (d["bin_pos"] + 0.5) * cw, y_of(d["p50_s"])
    ...
    with open(out_path, "w", encoding="utf-8") as fh:
        fh.write("\n".join(s))
    return {"colour_min_s": lo, "colour_max_s": hi,
