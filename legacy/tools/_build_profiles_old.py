...
def main() -> None:
    ...
    bins = pd.read_parquet(bins_paths)
    ...
    # drop duplicate sample_ids (rare shard-boundary leaks; rows are identical)
    samples_idx = samples.drop_duplicates("sample_id").set_index("sample_id")
    n_subs_link = np.ceil(samples_idx.L_link_m / L).clip(lower=1)
    bins["sub_id"] = np.clip(
