...
#
# 平台启动命令:
#   bash /nfs/dataset-ofs-494-1/project/user/junao/target_link/scripts/submit_train_smoke_job.sh
#   资源:1×GPU 单机(A100)
#
...
#   —— val_start(1320) = train_end(720) + W,正好卡着 tools/train_windows.py:79 的下界。
# 因为语料在本地,FETCH=0:完全不碰 HDFS。
#
# 真语料训练走 scripts/submit_train_windows_job.sh,不要改这个脚本。
set -euo pipefail
